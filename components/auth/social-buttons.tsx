...
export function SocialAuthButtons() {
  ...
  useEffect(() => {
    getProviders()
      .then((p) => setProviders({ google: !!p && "google" in p, apple: !!p && "apple" in p }))
      .catch(() => {});
  }, []);
      ...
      <div className="space-y-2">
        ...
        {providers.apple && (
          <button
            ...
            onClick={() => go("apple")}
            disabled={!!loading}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-black hover:bg-black/90 text-white text-sm font-medium rounded-lg border border-white/15 transition-colors disabled:opacity-60"
          >
            {loading === "apple" ? (
              <Loader2 size={16} className="animate-spin" />
            ) : (
